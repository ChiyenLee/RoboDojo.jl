function num_var(model::CentroidalQuadrupedParam) 
    nq = model.nq
    nc = model.nc 
    nb = 2
...
function indices_z(model::CentroidalQuadrupedParam) 
    nq = model.nq 
    nc = model.nc 
    nb = 2
    q = collect(1:nq) 
...
function indices_optimization(model::CentroidalQuadrupedParam) 
    nq = model.nq
    nc = model.nc 
    nb = 2
    nz = num_var(model) 
...
Trajectory(model::CentroidalQuadrupedParam, T) = Trajectory(model, T, nc=model.nc, nb=(2 * model.nc))
GradientTrajectory(model::CentroidalQuadrupedParam, T) = GradientTrajectory(model, T, nc=model.nc, nb=(2 * model.nc))
 
function residual(model::CentroidalQuadrupedParam, mass_matrix, dynamics_bias, kinematics, kinematics_jacobians, z, θ, μ)
...
function codegen_dynamics(model::CentroidalQuadrupedParam)
